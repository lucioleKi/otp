...
static Eterm erl_create_native_record(Process* p, Eterm* reg, Eterm id,
    ...
    if (is_non_value(cons)) {
        if (indicate_unloaded) {
            if (!erts_find_function(module, am_module_info, 0,
                                    erts_active_code_ix())) {
...
Eterm erl_create_native_record_jit(Process* p, Eterm* reg,
    ...
    res = erl_create_native_record(p, reg, id, live, size, new_p, true);
    if (res != NIL) {
        return res;
    }
...
Eterm erl_finish_create_record_jit(Process* p, Eterm* reg) {
    Eterm *E = p->stop;
    Eterm id;
    Uint live;
    Uint size;
    ...
    return erl_create_native_record(p, reg, id, live, size, new_p, false);
}
...
Eterm erl_finish_create_record_emu(Process* p, Eterm* reg) {
    ...
    /* The state is now completely restored. Again attempt to create
     * the record. */
    res = erl_create_native_record(p, reg, id, live, size, new_p, false);
    if (is_value(res)) {
        reg[1023] = res;
        return cp;
    } else {
