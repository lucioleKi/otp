...
opt_records([{get_record_elements_id,Fail,Id,Src,List}=I|Is], Acc0) ->
    case simplify_get_record_elements_id(Fail, Id, Src, List, Acc0) of
        {ok,Acc} ->
            opt_records(Is, Acc);
        error ->
    ...
    case not is_reg_overwritten(Src, List1) andalso
        ...
        true ->
            case member(Key, List1) of
                true ->
                    %% The key is already in the other list. That is
                    %% very unusual, because there are optimizations
                    %% to get rid of duplicate keys. Therefore, don't
                    %% try to do anything smart here; just keep the
                    %% get_record_elements_id instructions separate.
                    error;
                false ->
                    List = [Key,Dst|List1],
                    {ok,[{get_record_elements_id,Fail,Id,
                          Src,{list,List}}|Acc]}
            end;
        false ->
            %% A destination is used more than once. That should only
...
simplify_get_record_elements_id(_, _, _, _, _) -> error.
