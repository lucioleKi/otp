...
clash(Config) when is_list(Config) ->
    ...
    true = code:add_path(DDir++"foobar-0.1/ebin"),
    true = code:add_path(DDir++"foobar-0.2/ebin"),
    ct:capture_start(),
    ok = code:clash(),
    ct:capture_stop(),
    ...
    %% TODO: Change this to move a directory instead of moving a file
    % Priv = proplists:get_value(priv_dir, Config),
    % TmpEzFile = Priv++"baz.beam",
    % {ok, _} = file:copy(DDir++"foobar-0.1/baz.beam", TmpEzFile),
