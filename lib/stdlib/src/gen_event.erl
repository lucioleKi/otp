...
This behavior module provides event handling functionality. It consists of a
generic event manager process with any number of event handlers that are added
and deleted dynamically.

An event manager implemented using this module has a standard set of interface
functions and includes functionality for tracing and error reporting. It also
fits into an OTP supervision tree. For more information, see
[OTP Design Principles](`e:system:events.md`).

Each event handler is implemented as a callback module exporting a predefined
...
calling callback function [`Module:terminate/2`](`c:terminate/2`), giving as
argument `{error,{'EXIT',Reason}}` or `{error,Term}`, respectively. No other
event handler is affected.

A `gen_event` process handles system messages as described in `m:sys`. The `sys`
...
event manager does not exist or if bad arguments are specified.
...
> signaling can, for example, cause call timeouts in `gen_event` to be
...
- If `{swap_handler,Args1,NewState,Handler2,Args2}` is returned, the event
  handler is replaced by `Handler2` by first calling
  [`Module:terminate(Args1,NewState)`](`c:terminate/2`) and then
  [`Module2:init({Args2,Term})`](`c:init/1`), where `Term` is the return value
  of [`Module:terminate/2`](`c:terminate/2`). For more information, see
  `swap_handler/3`.
...
-callback handle_event(Event :: term(), State :: term()) ->
    ...
    remove_handler.
-doc """
Whenever an event manager receives a request sent using [`call/3,4`](`call/3`),
...
[`Module:handle_event/2`](`c:handle_event/2`) except that they also contain a
...
ordered by its supervisor to terminate. Even if it is _not_ part of a
...
- The event handler terminates abnormally and `gen_event` logs an error. `Opt`
...
- When an event handler terminates abnormally, `gen_event` logs `Status` in
...
manager is killed with [`exit(_, kill)`](`erlang:exit/2`).
...
> #### Warning {: .warning }
>
> Before OTP 26.0, if the started event manager failed to register its name,
> this founction could return `{error,{already_started,OtherPid}}` _before_ the
> started event manager process had terminated so starting again might fail
> because the registered name was not yet unregistered, and an `'EXIT'` message
> could arrive later to the process calling this function.
>
> But if the start timed out, this function killed the started event manager
> process and returned `{error,timeout}`, and then the process link
> `{'EXIT',Pid,killed}` message _was_ consumed.
>
> The start was made synchronous in OTP 26.0 and the guarantee was implemented
> that no process link `'EXIT'` message from a failed start will linger in the
> caller's inbox.
""".
-doc(#{since => <<"OTP 20.0">>}).
-spec start_link(emgr_name(), [option()]) -> start_ret().
...
If [`Module:init/1`](`c:init/1`) returns a correct value indicating successful
completion, the event manager adds the event handler and this function returns
`ok`. If [`Module:init/1`](`c:init/1`) fails with `Reason` or returns
...
- If the calling process later terminates with `Reason`, the event manager
  deletes any supervised event handlers by calling
...
- If the event handler is deleted later, the event manager sends a message
  ...
  - `normal`, if the event handler has been removed because of a call to
    [`delete_handler/3`](`delete_handler/3`), or `remove_handler` has been
    returned by a callback function (see below).
  - `shutdown`, if the event handler has been removed because the event manager
    is terminating.
  - `{swapped,NewHandler,Pid}`, if the process `Pid` has replaced the event
    ...
    `swap_handler/3` or `swap_sup_handler/3`.
...
[`Module:handle_call/2`](`c:handle_call/2`). If the specified event handler is
...
[`gen_event:call(EventMgrRef, Handler, Request, Timeout)`](`call/3`), ignoring
the error handling.

The event manager calls [`Module:handle_call/2`](`c:handle_call/2`) to handle
the request.
...
The return value is the return value of [`Module:terminate/2`](`c:terminate/2`).
If the specified event handler is not installed, the function returns
