...
[`cursor(QH)`](`cursor/1`) is equivalent to [`cursor(QH, [])`](`cursor/2`).
""".
...
[`eval(QH)`](`eval/1`) is equivalent to [`eval(QH, [])`](`eval/2`).
""".
...
extra argument `AccIn`. The query handle and the function are evaluated in the
...
[`fold(Function, Acc0, QH)`](`fold/3`) is equivalent to
[`fold(Function, Acc0, QH, [])`](`fold/4`).
""".
...
[`info(QH)`](`info/1`) is equivalent to [`info(QH, [])`](`info/2`).
...
[`keysort(KeyPos, QH1)`](`keysort/2`) is equivalent to
[`keysort(KeyPos, QH1, [])`](`keysort/3`).
""".
...
returned. Defaults to `10`. If less than the requested number of answers is
...
[`q(QLC)`](`q/1`) is equivalent to [`q(QLC, [])`](`q/2`).
...
[`sort(QH1)`](`sort/1`) is equivalent to [`sort(QH1, [])`](`sort/2`).
""".
...
[`string_to_handle(QueryString)`](`string_to_handle/1`) is equivalent to
[`string_to_handle(QueryString, [])`](`string_to_handle/2`).

[`string_to_handle(QueryString, Options)`](`string_to_handle/2`) is equivalent
to
[`string_to_handle(QueryString, Options, erl_eval:new_bindings())`](`string_to_handle/3`).
...
the data structure create a query handle by calling `qlc:table/2`. The different
ways to traverse the table and properties of the table are handled by callback
